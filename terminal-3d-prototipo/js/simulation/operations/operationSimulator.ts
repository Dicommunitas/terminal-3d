...
import { dataSimulator, DataUpdateEvent } from "./dataSimulator";
...
interface BaseOperationParams {
    operationId?: string; // Opcional, será gerado se não fornecido
}
...
export interface TransferProductParams extends BaseOperationParams {
    ...
    pipeId: string;
    valveIds: string[]; // Válvulas no caminho que precisam estar abertas
    transferRate: number; // Unidades por segundo (ex: m³/s)
    targetVolume?: number; // Volume a transferir (opcional)
    duration?: number; // Duração da transferência em segundos (opcional)
}
...
export class OperationSimulator {
    private static _instance: OperationSimulator;
    private _activeOperations: Map<string, OperationStatus> = new Map();
    private _operationTimers: Map<string, any> = new Map(); // Para operações baseadas em tempo

    // Observável para notificar sobre mudanças no status das operações
    public onOperationStatusChangeObservable: Observable<OperationStatus> = new Observable<OperationStatus>();

    /**
     * Obtém a instância única do OperationSimulator (Singleton)
     */
    public static getInstance(): OperationSimulator {
     ...
     */
    public startSetValveState(params: SetValveStateParams): string | null {
        const valve = db.getEquipmentById(params.valveId) as ValveData;
        ...
        this._operationTimers.set(operationId, setTimeout(() => {
            const currentValve = db.getEquipmentById(params.valveId) as ValveData;
            if (currentValve) {
     ...
     * @returns O ID da operação iniciada ou null em caso de erro.
     */
    public startTransferProduct(params: TransferProductParams): string | null {
        const sourceTank = db.getEquipmentById(params.sourceTankId) as TankData;
        const destTank = db.getEquipmentById(params.destinationTankId) as TankData;
        const pipe = db.getEquipmentById(params.pipeId) as PipeData;

        if (!sourceTank || sourceTank.type !== "tank") return this._logOperationError(params, `Tanque de origem ${params.sourceTankId} não encontrado.`);
        if (!destTank || destTank.type !== "tank") return this._logOperationError(params, `Tanque de destino ${params.destinationTankId} não encontrado.`);
        if (!pipe || pipe.type !== "pipe") return this._logOperationError(params, `Tubulação ${params.pipeId} não encontrada.`);
        if (params.transferRate <= 0) return this._logOperationError(params, `Taxa de transferência deve ser positiva.`);
        if (!params.duration && !params.targetVolume) return this._logOperationError(params, `Duração ou volume alvo deve ser especificado.`);

        // Verificar se as válvulas no caminho estão abertas
        for (const valveId of params.valveIds) {
            const valve = db.getEquipmentById(valveId) as ValveData;
            if (!valve || valve.type !== "valve") return this._logOperationError(params, `Válvula ${valveId} não encontrada.`);
            if (valve.state !== "open") return this._logOperationError(params, `Válvula ${valveId} não está aberta.`);
        }
        ...
        const intervalId = setInterval(() => {
            ...
            const currentSourceTank = db.getEquipmentById(params.sourceTankId) as TankData;
            const currentDestTank = db.getEquipmentById(params.destinationTankId) as TankData;
            const currentPipe = db.getEquipmentById(params.pipeId) as PipeData;
    ...
    public cancelOperation(operationId: string): boolean {
        ...
        if (status.type === "transferProduct") {
             ...
             if (pipeId) {
                 const pipe = db.getEquipmentById(pipeId) as PipeData;
     ...
     */
    public getActiveOperations(): OperationStatus[] {
     ...
     * @private
     */
    private _updateOperationStatus(operationId: string, status: OperationStatus["status"], message?: string, progress?: number): void {
        const currentStatus = this._activeOperations.get(operationId);
        if (currentStatus) {
     ...
     * @private
     */
    private _logOperationError(params: BaseOperationParams, errorMessage: string): null {
...
(window as any).OperationSimulator = operationSimulator;
