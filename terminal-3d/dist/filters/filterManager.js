...
export class FilterManager {
    ...
    updateFilterSet(id, updates) {
        ...
        Object.assign(filterSet, Object.assign(Object.assign({}, updates), { dateModified: new Date() }));
    ...
    applyFilters(filterSet) {
        ...
        if (filterSet.filters && filterSet.filters.length > 0) {
            ...
            if (activeFilters.length > 0) {
                filteredEquipment = filteredEquipment.filter(equipment => activeFilters.every(filter => filter.filter(equipment)));
            }
        }
        ...
        if (filterSet.stateFilter && filterSet.stateFilter.isActive && filterSet.stateFilter.states.length > 0) {
            ...
            filteredEquipment = filteredEquipment.filter(equipment => equipment.status && states.has(equipment.status));
        }
        ...
        if (filterSet.productFilter && filterSet.productFilter.isActive && filterSet.productFilter.products.length > 0) {
            ...
            filteredEquipment = filteredEquipment.filter(equipment => 'product' in equipment && equipment.product && products.has(equipment.product));
        }
        ...
        if (filterSet.propertyFilters && filterSet.propertyFilters.length > 0) {
            ...
            activePropertyFilters.forEach(propFilter => {
                filteredEquipment = filteredEquipment.filter(equipment => {
                    const value = equipment[propFilter.property];
                    if (value === undefined)
                        return false;
     ...
     */
    createTextFilter(searchText, fields = []) {
        return {
            searchText,
            fields,
            isActive: true
     ...
     */
    createSpatialFilter(center, radius) {
        return {
            center,
            radius,
            isActive: true
     ...
     */
    createPropertyFilter(property, operator, value, value2) {
        return {
            property,
            operator,
            value,
     ...
     */
    createCategoryFilter(categoryId, includeSubcategories = true) {
        return {
            categoryId,
     ...
     */
    createCustomFilter(id, name, filterFn, description) {
        return {
            id,
            name,
            description,
