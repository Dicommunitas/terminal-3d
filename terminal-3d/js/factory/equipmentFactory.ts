import { Scene, Vector3, TransformNode, Mesh, MeshBuilder, PBRMaterial, StandardMaterial, Color3, Nullable } from "@babylonjs/core";
import { db, TankData, PipeData, ValveData, LoadingAreaData, EquipmentDataUnion } from "../database/inMemoryDb";
...
export interface EquipmentMetadata {
    id: string;
    type: string; // 'tank', 'pipe', 'valve', 'loadingArea'
    equipmentType?: string; // Subtipo específico ('standard', 'large', etc.)
    data: EquipmentDataUnion; // Referência aos dados completos do DB
}
...
export interface EquipmentCatalogItem {
    id: string;
    type: string; // 'tank', 'pipe', 'valve', 'loadingArea'
    subtype: string; // 'standard', 'large', 'gate', etc.
    name: string;
    description?: string;
    tags: string[];
    previewImage?: string;
    defaultProperties: any; // Propriedades padrão para este tipo de equipamento
    dateAdded: Date;
    dateModified: Date;
    usageCount: number; // Contador de uso para estatísticas
}
...
export class EquipmentFactory {
    private static _instance: EquipmentFactory;
    private _scene: Nullable<Scene> = null;
    private _groups: Nullable<EquipmentGroups> = null;
    private _catalog: Map<string, EquipmentCatalogItem> = new Map();
    private _catalogByType: Map<string, string[]> = new Map(); // Tipo -> [id1, id2, ...]
    private _catalogByTag: Map<string, string[]> = new Map(); // Tag -> [id1, id2, ...]

    // Configurações para os equipamentos
    private readonly _config: EquipmentConfig = {
        tanks: {
            defaultMaterial: {
     ...
     */
    public initialize(scene: Scene, groups: EquipmentGroups): void {
        this._scene = scene;
        this._groups = groups;
     ...
     */
    private _addCatalogItem(item: EquipmentCatalogItem): void {
     ...
     */
    public createEquipment(equipmentData: EquipmentDataUnion): Nullable<TransformNode> {
        ...
        try {
            ...
            switch (equipmentData.type) {
                case "tank":
                    return this._createTank(equipmentData as TankData);
                case "pipe":
                    return this._createPipe(equipmentData as PipeData);
                case "valve":
                    return this._createValve(equipmentData as ValveData);
                case "loadingArea":
                    ...
                    return null;
                default:
                    console.warn(`Tipo de equipamento desconhecido: ${(equipmentData as any).type}`);
                    return null;
            }
        } catch (error) {
     ...
     * @returns O nó do equipamento criado ou null em caso de erro
     */
    public createFromCatalog(catalogId: string, position: Vector3, customProperties: any = {}): Nullable<TransformNode> {
        ...
        const equipmentData: any = {
            ...catalogItem.defaultProperties,
            ...customProperties,
            id: uniqueId,
            type: catalogItem.type,
            position: position,
            catalogId: catalogId // Referência ao item do catálogo
        };
        ...
        // Criar o equipamento
        const equipmentNode = this.createEquipment(equipmentData as EquipmentDataUnion);
     ...
     */
    private _createTank(tankData: TankData): Nullable<TransformNode> {
        if (!this._groups?.tanks) {
     ...
     */
    private _createPipe(pipeData: PipeData): Nullable<TransformNode> {
        if (!this._groups?.pipes) {
     ...
     */
    private _createValve(valveData: ValveData): Nullable<TransformNode> {
        if (!this._groups?.valves) {
     ...
     */
    public getAllCatalogItems(): EquipmentCatalogItem[] {
     ...
     */
    public getCatalogItemsByType(type: string): EquipmentCatalogItem[] {
        const ids = this._catalogByType.get(type) || [];
        return ids.map(id => this._catalog.get(id)).filter(Boolean) as EquipmentCatalogItem[];
    }
     ...
     */
    public getCatalogItemsByTag(tag: string): EquipmentCatalogItem[] {
     ...
     */
    public searchCatalog(searchText: string): EquipmentCatalogItem[] {
        const searchLower = searchText.toLowerCase();
        return Array.from(this._catalog.values()).filter(item => 
     ...
     */
    public addCatalogItem(item: EquipmentCatalogItem): boolean {
     ...
     */
    public updateCatalogItem(id: string, updates: Partial<EquipmentCatalogItem>): boolean {
        ...
        const updatedItem = { ...item, ...updates, dateModified: new Date() };
     ...
     */
    public removeCatalogItem(id: string): boolean {
        const item = this._catalog.get(id);
        if (!item) {
            console.warn(`Item com ID ${id} não encontrado no catálogo.`);
            return false;
...
(window as any).EquipmentFactory = equipmentFactory;
