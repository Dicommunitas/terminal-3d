import { Scene, AbstractMesh, PointerInfo, PointerEventTypes, HighlightLayer, Color3, Observer, Nullable, Sound, Vector3, TransformNode, PickingInfo, Mesh, Node } from "@babylonjs/core"; // Added Mesh, Node
...
export class ObjectPicker {
    ...
    private _handlePointerMove(pointerInfo: PointerInfo): void {
        ...
        let targetObject: Nullable<AbstractMesh | TransformNode> = null;
            ...
            if (this._hoveredObject) {
                ...
                this._tooltipTimeout = setTimeout(() => {
                    if (this._hoveredObject) { // Check again in case mouse moved out quickly
                       this._showTooltip(this._hoveredObject, pointerInfo.event as MouseEvent);
                    }
    ...
    private _processSelection(targetObject: AbstractMesh | TransformNode): void {
        if (targetObject === this._selectedObject) return; // Already selected
    ...
    private _showTooltip(targetObject: AbstractMesh | TransformNode, event: MouseEvent): void {
        ...
        if (!tooltipContent) return; // Don't show empty tooltip
    ...
    public selectObjectById(objectId: string): boolean {
        ...
        let targetNode: Nullable<AbstractMesh | TransformNode> = null;
        ...
        if (targetNode) {
             ...
             if (objectToSelect) { // Check if null
                 ...
                 if (isPickable || (objectToSelect as any).metadata) {
                     this._processSelection(objectToSelect);
        ...
        for (const mesh of this._scene.meshes) {
            ...
            if (objectWithMetadata && (objectWithMetadata as any).metadata?.id === objectId) {
     ...
     */
    private _findObjectWithMetadata(startNode: AbstractMesh | TransformNode): Nullable<AbstractMesh | TransformNode> {
        let current: Nullable<Node> = startNode;
        while (current) {
            // Check if the current node has metadata
            if ((current as any).metadata?.id) {
    ...
    private _addHighlight(targetObject: AbstractMesh | TransformNode): void {
        try {
            if (targetObject instanceof Mesh) {
                ...
                targetObject.getChildMeshes(false).forEach(mesh => { // false = direct descendants only
