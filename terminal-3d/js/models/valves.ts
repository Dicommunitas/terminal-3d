...
export class ValvesManager {
     ...
     */
    public createOrUpdateValve(valveData: DbValveData): Nullable<InstancedMesh> {
        if (!this._valvesGroup) {
        ...
        let valveBodyInstance: InstancedMesh;
        let wheelOrLeverInstance: InstancedMesh;
        let stateIndicatorInstance: Nullable<InstancedMesh> = null;
     ...
     */
    public setValveStateVisual(valveId: string, state: ValveState): void {
        ...
        if (metadata) {
            ...
            Object.values(metadata.components).forEach(instance => {
                if (instance) {
                    ...
                    if (instance.material && instance.material !== this._instanceMaterials[metadata.valveType] && !Object.values(this._instanceMaterials).includes(instance.material as StandardMaterial)) {
     ...
     */
    public getValveInstance(valveId: string): Nullable<InstancedMesh> {
        return this._valveInstances.get(valveId)?.components.body || null;
    }
}
